...
def load_models_no_segments():
    ...
    for class_ in ["85738", "85728", "85721", "85714", "85696", "85568"]:
        models[class_] = pickle.load(
            open(
                path_models
                / f"experiment_data_one_class_{class_}.pkl",
                "rb",
            )
        )
    return models
...
def main_classification_multiple_texts_no_segments(texts: list, codigos: list, ids: list):
    dic_models = load_models_no_segments()
    results = []
    codigos_ = []
    ids_ = []
    for index, text in enumerate(texts):
        X = hashing_texts([text], n_features=12000)
        results_dic = {k: 0 for k in dic_models.keys()}
        for class_, model in dic_models.items():
            results_dic[class_] = model.predict_proba(X)[0][1]
        results.append(results_dic)
    ...
    for row in df.to_dict("records"):
        ...
        new_rows.append(dic_aux)
    df = pd.DataFrame(new_rows)
    df.to_csv(f"D:\\TJSP_clustering_data\\acordaos_principais_3k_classes_no_segments_results.csv")
...
if __name__ == "__main__":
    ...
    main_classification_multiple_texts_no_segments(
        df["conteudo"].tolist(),
        df["codigos_movimentos_temas"].tolist(),
        df["id_documento"].tolist(),
    )
