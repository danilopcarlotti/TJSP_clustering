...
if __name__ == '__main__':
    ...
    requiredArgs.add_argument('-s','--source',
                    help = 'especifica o arquivo a ser processado.',required=True,metavar = '')    
    optionalArgs = ap.add_argument_group('Argumentos Opcionais')
    
    optionalArgs.add_argument('-c', '--col', default=-1,
                    help = 'especifica a coluna a ser processada. Se nenhuma ' +
                           ' coluna for especificada, a coluna com os textos ' +
                           'será identificada de modo automático.',metavar= '')
    
    optionalArgs.add_argument('-d', '--dest', default='',
                    help = 'especifica o diretório para onde será salvo ' + 
                           ...
                           'mesmo diretório do arquivo a ser processado.',metavar= '')
    optionalArgs.add_argument('-i',type = int,default = 0,
                    help = 'especifica a partir de que linha do csv especificado ' + 
                           'o processamento deve iniciar.',metavar= '')
    optionalArgs.add_argument('-n',type = int, default = -1,
                    help = 'especifica a quantidade de linhas a serem processadas. '+
                           'Se nenhuma quantidade for especificada, todas as linhas '+
                           'serão processadas, ignorando o argumento -i.',metavar= '')
    ...
    for i in range(firstRow,lastRow):
        try:
            df.iloc[i, textCol] = removeHeaderFooter(df.iloc[i,textCol])
    ...
    fileName = '_' + os.path.basename(args['source'])
