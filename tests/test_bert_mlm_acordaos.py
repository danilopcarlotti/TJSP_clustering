...
class BERT_MLM_eval:
    ...
    tokenizer = BertTokenizer.from_pretrained(
        ...
        cache_dir="/home/padilha/DATA2/pytorch/hub/",
    )
    ...
    def eval_15(self, input_text):
        ...
        mask_arr = (
            (rand < 0.15) * (tkn_input.input_ids != 101) * (tkn_input.input_ids != 102)
        )
        _, mask_indexes = mask_arr.nonzero(as_tuple=True)
        original_masked_tkn = self.tokenizer.convert_ids_to_tokens(
            tkn_input.input_ids[0, mask_indexes]
        )
