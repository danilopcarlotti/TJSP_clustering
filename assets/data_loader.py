...
class DataLoader:
    ...
    def __init__(
        self, file_path="../data/base_acordaos2.parquet", theme_id="all", stats=True
    ):
        ...
        print(df.shape)
        df.loc[:, "number_of_themes"] = df[
            ["S0929", "S1015", "S1033", "S1039", "S1046", "S1101"]
        ].sum(axis="columns", numeric_only=True)

        # filter out documents with more than one theme
        if DataLoader.df is None:
            DataLoader.df = df.loc[df["number_of_themes"] == 1]
            DataLoader.df.loc[:, "theme"] = DataLoader.df[
                ["S0929", "S1015", "S1033", "S1039", "S1046", "S1101"]
            ].idxmax(axis="columns")
        ...
        if stats:
            print("Themes count:")
            print(
                DataLoader.df[["S0929", "S1015", "S1033", "S1039", "S1046", "S1101"]]
    ...
    def _is_valid_theme(self, theme_id):
        if theme_id not in [
            ...
            raise ValueError(
                "Invalid theme! Valid themes are 'S0929', 'S1015', 'S1033', 'S1039', 'S1046', 'S1101' or 'all'"
            )
    ...
    def __iter__(self):
        if self.theme_id != "all":
            self.df_iter = DataLoader.df.loc[
                DataLoader.df[self.theme_id] == 1
            ].iterrows()
        else:
